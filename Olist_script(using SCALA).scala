...
var spark = SparkSession.builder.appName("ETL-pipeline-using-Airflow-and-AWS-EMR-cluster").getOrCreate()
...
var items = spark.read.csv("s3a://olist-project/data/olist_order_items_dataset.csv",header=True,inferSchema=True).cache()
var orders = spark.read.csv("s3a://olist-project/data/olist_orders_dataset.csv",header=True,inferSchema=True).cache()
var products = spark.read.csv("s3a://olist-project/data/olist_products_dataset.csv",header=True,inferSchema=True)
...
var orders_info = items.join(orders,items.order_id == orders.order_id)\
